...
class LogService {
  constructor() {
    log.transports.console.level = require$$0$5.app.isPackaged ? "info" : "debug";
    log.transports.file.level = "info";
    log.transports.file.maxSize = 1024 * 1024 * 10;
    const logFolder = require$$0$5.app.isPackaged ? path.join(require$$0$5.app.getPath("userData"), "logs") : path.join(process.cwd(), "logs");
  ...
  openLogFolder() {
    ...
    require$$0$5.shell.showItemInFolder(logPath);
  }
}
...
class WindowService {
  ...
  createWindow(type, routePath = "", customOptions) {
    ...
    if (preset.parent && typeof preset.parent === "string") {
      const parentWin = this.windowMap.get(preset.parent);
      if (parentWin) {
...
require$$0$5.ipcMain.on("log-to-main", (_, { level, message, args }) => {
...
require$$0$5.ipcMain.on("login-success", () => {
